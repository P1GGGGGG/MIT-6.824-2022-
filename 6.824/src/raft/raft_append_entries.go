...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex < rf.lastSnapshotIndex {
		...
		rf.logs = rf.logs[:rf.getStoreIndexByLogIndex(index+1)]
		reply.Success = false
	...
	if reply.Success {
		...
		if rf.commitIndex < args.LeaderCommit {
			if _, lastLogIndex := rf.getLastLogTermAndIndex(); args.LeaderCommit > lastLogIndex {
...
func (rf *Raft) tryCommitLog() {
	...
	hasCommit := false
	flag := false
	// 从后往前遍历，只能提交当前term的,从而间接提交之前term的
	for i := lastLogIndex; i > rf.commitIndex; i-- {
		if flag {
			break
		}
		now_index := rf.getStoreIndexByLogIndex(i)
		if rf.logs[now_index].Term == rf.currentTerm {
			count := 0
